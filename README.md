# Bienvenue dans notre jeu Snake personnalisé !

Notre idée de jeu : créer une version de base qui correspond au jeu Snake, puis ajouter des variantes telles que "pour grandir il faut seulement manger les nombres qui sont des multiples de 3", etc. Une autre variante pourrait typiquement être un pacman ! On enlève la dimension de changement de taille, et on peut ajouter des ennemis par exemple.

## Arborescence conseillée du dépôt
.<br>
├── README.md                     # sous-domaine,périmètre, intention<br>
├── docs/<br>
│   ├── state-of-the-art.md       # ≥5 refs + tableau comparatif + originalité<br>
│   ├── services-notes.md         # attentes qualitatives par service (2–3 lignes)<br>
│   └── VARIABILITY.md            # liste paramètres CT/RT + presets + scénarios<br>
├── model/<br>
│   ├── metamodel.puml            # PlantUML<br>
├── lang/<br>
│   └── grammar.langium           # export du playground (brouillon)<br>
├── examples/<br>
│   ├── hello.dsl<br>
│   ├── variantA.dsl<br>
│   └── variantB.dsl<br>
└── OPEN-QUESTIONS.md<br>

## HOW-TO SNAKE :
Rédiger clairement içi comment générer, où sont les variantes, validations, preview.
Si ça veut dire qqch...